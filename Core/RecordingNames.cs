...
namespace SnapView.Core
{
    ...
    {
        internal const string DefaultPattern = CaptureNames.FramePattern;
        ...
        {
            return CaptureNames.Build(string.IsNullOrWhiteSpace(pattern) ? DefaultPattern : pattern,
                when, Sanitize(subject, MaxSubjectLength));
        }
